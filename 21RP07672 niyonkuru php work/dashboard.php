...
if ($_SESSION['data'] == false) {
	header("location:index.php?ERROR_loginFirst");
}
...
$namerr = $regnoerr =  $sexerr = $ageerr = $classerr = "";
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if (empty($name)) {
			...
			$sql = mysqli_query($link,"INSERT INTO students(name, regno, sex, age, class) values ('$name','$regno','$sex','$age','$class')");
...
<center><h3>Record  student</h3></center>
	<form method="post" action="">
		...
		<label>	Sex:</label>
		<input type="radio" name="sex" value="M">Female
		<input type="radio" name="sex" value="F">Male<br>
<br>
		<label>	Age:</label>
...
<h3>Recorded Students</h3>



		<?php

			$sql2 = mysqli_query($link,"select * from students");
