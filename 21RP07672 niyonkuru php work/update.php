...
$namerr = $regnoerr =  $sexerr = $ageerr = $classerr = "";
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if (empty($name)) {
			...
			$sql = mysqli_query($link,"update students set name='$name', regno='$regno', sex='$sex', age='$age', class='$class'where regno = '$regno'");
...
<head>
	...
	<?php
	$id = $_GET['id'];
$sql2 = mysqli_query($link, "select * from students where id = '$id'");
...
<center><h3>Update student</h3></center>
	<form method="post" action="">
		...
		<input type="radio" name="sex" value="F">Male
